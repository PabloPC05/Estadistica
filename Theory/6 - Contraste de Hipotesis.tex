...
\begin{definición}[Familia de distribuciones de razón de verosimilitud monótona]
    Una familia $\{f_\theta : \theta \in \Theta\}$ tiene razón de verosimilitud monótona (RLVM) respecto a $T(\vec{X}) \text{ si, para } \theta_1 < \theta_2$ la razón $\frac{f_{\theta_2}(\vec{x})}{f_{\theta_1}(\vec{x})}$ es monótona en $T(\vec{x})$.
\end{definición}
